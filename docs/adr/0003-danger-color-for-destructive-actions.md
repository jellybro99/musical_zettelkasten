...
- **Scope the new token to a confirm modal instead of the trigger button** — rejected at the time: the ticket motivating this change kept the existing `window.confirm(...)` call and did not introduce a new confirm-modal component, so the danger color belonged on the button that triggers deletion, not on UI that didn't exist. Superseded by a later ticket (arrangement-ui-polish #05), which does introduce a shared `ConfirmDialog` component to replace `window.confirm()` app-wide; that dialog's destructive action button now also carries `.btn-danger`, visually separated from the dialog's other button(s). The trigger-button color itself is unaffected by this.
